...
- [ ] Experiment on GAIL
  - My intention is to make the agent not interact with the environment during training (i.e. work in an **offline** setting), given a dataset generated by the expert that only contains tuples in the form of (state, action, next_state) (i.e. no reward signals). We need to figure out how to achieve this.
