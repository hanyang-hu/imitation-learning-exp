...
- [x] Collect data of manual control and dump it into a pickle file (currently, `transition_data.pkl` contains 11339 state-action pairs)
  - The distribution of actions in `transition_data.pkl` is almost uniform, this should not happen. The api is not recording the manual control data, although this dataset can still be used as a task-irrelevant prior in an offline reinforcement learning setting. We need to collect the dataset again :(
